"""Production platform foundation: tenancy, identity, jobs, rules and connectors."""
...
def upgrade():
    for table in ("events", "incidents", "activity"):
    ...
    op.create_index("ix_approval_tenant_status", "action_approvals", ["tenant_id", "status"])
    op.create_table(
        "ingestion_jobs",
        sa.Column("id", sa.String(), primary_key=True),
        sa.Column("tenant_id", sa.String(), nullable=False, index=True),
    ...
    op.create_index("ix_job_tenant_status", "ingestion_jobs", ["tenant_id", "status"])
    op.create_table(
        "detection_rules",
    ...
    op.create_index("ix_rule_status", "detection_rules", ["status"])
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.UniqueConstraint("idempotency_key", name="uq_connector_idempotency"),
    )
