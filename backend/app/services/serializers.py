...
def serialize_event(event):
    return {
        ...
        "risk_flags": json.loads(event.risk_flags),
...
def serialize_incident(incident, events=None, db=None):
    result = {
        ...
        "confidence_breakdown": json.loads(incident.confidence_breakdown or "{}"),
    ...
    if db is not None:
        ...
        result["notes"] = [
            ...
            for n in db.query(AnalystNote)
            .filter_by(incident_id=incident.id)
            .order_by(AnalystNote.timestamp.desc())
        ]
        result["bookmarks"] = [
            {
                "id": b.id,
            ...
            for b in db.query(EvidenceBookmark)
            .filter_by(incident_id=incident.id)
            .order_by(EvidenceBookmark.timestamp.desc())
        ]
        result["risk_history"] = [
            {
                "timestamp": h.timestamp.isoformat() + "Z",
                "original_risk": h.original_risk,
                "residual_risk": h.residual_risk,
                "reason": h.reason,
            }
            for h in db.query(IncidentRiskHistory)
            .filter_by(incident_id=incident.id)
        ...
        result["detection_findings"] = (
            [
                ...
                for f in db.query(DetectionFindingRecord)
                .filter(DetectionFindingRecord.event_id.in_(ids))
                .order_by(DetectionFindingRecord.event_id, DetectionFindingRecord.rule_id)
            ]
            if ids
            else []
        )
