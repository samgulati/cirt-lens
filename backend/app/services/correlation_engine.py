...
def fingerprint(incident_type,principal,start):
    bucket=int(start.timestamp()//(settings.max_incident_span_minutes*60));return hashlib.sha256(f"{incident_type}|{principal}|{bucket}".encode()).hexdigest()[:24]
...
def create_or_update_incidents(db,suspicious_events,findings):
    ...
    for group in connected_groups(suspicious_events):
        ...
        users=[e.user for e in group if e.user];hosts=[e.host for e in group if e.host];principal=Counter(users or hosts).most_common(1)[0][0];incident_type,title,root=infer_incident_type(flags);fp=fingerprint(incident_type,principal,group[0].timestamp)
        existing=db.query(Incident).filter(Incident.incident_fingerprint==fp,Incident.status.in_(["NEW","INVESTIGATING"])).first()
        if not existing:
            # A later batch can strengthen a generic case and change its
            # classification/fingerprint. Reuse an open case sharing evidence.
            for candidate in db.query(Incident).filter(Incident.status.in_(["NEW","INVESTIGATING"])).all():
        ...
        if existing:
            ...
            for e in db.query(Event).filter(Event.id.in_(combined)):existing_events[e.id]=e
        ...
        flags=[f.flag for f in group_findings];users=[e.user for e in all_group if e.user];hosts=[e.host for e in all_group if e.host];principal=Counter(users or hosts).most_common(1)[0][0];incident_type,title,root=infer_incident_type(flags);fp=fingerprint(incident_type,principal,all_group[0].timestamp)
        ...
        if existing:
            ...
            if db.query(Incident).filter(Incident.incident_fingerprint==fp,Incident.id!=existing.id).first():fp=existing.incident_fingerprint
        ...
        if db.query(Incident).filter(Incident.incident_fingerprint==fp).first():fp=hashlib.sha256(f"{fp}|{'|'.join(ids)}".encode()).hexdigest()[:24]
