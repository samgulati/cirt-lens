...
def issue_token(user):
    ...
    claims = {
        ...
        "iss": settings.oidc_issuer or "cirt-lens-local",
    }
...
def seed_identity(db):
    ...
    if not db.get(Tenant, settings.default_tenant_id):
        db.add(
            Tenant(
                id=settings.default_tenant_id, name="CIRT Lens Demo Organization", created_at=now
            )
        )
    demos = [
        ("viewer@demo.local", "Viewer", "VIEWER"),
        ("analyst@demo.local", "Analyst", "ANALYST"),
        ("responder@demo.local", "Responder", "RESPONDER"),
        ("admin@demo.local", "Administrator", "ADMINISTRATOR"),
    ]
    for email, name, role in demos:
        if (
            not db.query(UserAccount)
            .filter_by(tenant_id=settings.default_tenant_id, email=email)
            .first()
        ):
            db.add(
                UserAccount(
...
def current_principal(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if settings.auth_mode == "oidc":
            ...
            signing_key = (
                jwt.PyJWKClient(settings.oidc_jwks_url).get_signing_key_from_jwt(token).key
            )
            claims = jwt.decode(
                token,
                signing_key,
            ...
            claims = jwt.decode(
                ...
                algorithms=["HS256"],
                audience=settings.oidc_audience,
                issuer=settings.oidc_issuer or "cirt-lens-local",
            )
        ...
        if not tenant_id or not roles:
            raise ValueError("Required tenant or role claims are missing")
        role = max(
            (str(role).upper() for role in roles), key=lambda value: ROLE_LEVEL.get(value, -1)
        )
        return Principal(
            claims["sub"], tenant_id, claims.get("email", claims["sub"]), role, permissions
        )
...
def require_role(minimum):
    def dependency(principal: Principal = Depends(current_principal)):
        if ROLE_LEVEL.get(principal.role, -1) < ROLE_LEVEL[minimum]:
