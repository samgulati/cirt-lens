...
from .auth import Principal,current_principal,require_role,issue_token,verify_password
...
def request_approval(incident_id:str,body:ApprovalRequest,p:Principal=Depends(require_role("RESPONDER")),db:Session=Depends(get_db)):
...
def approve(approval_id:str,p:Principal=Depends(require_role("ADMINISTRATOR")),db:Session=Depends(get_db)):
...
def approvals(p:Principal=Depends(require_role("RESPONDER")),db:Session=Depends(get_db)):return [{"id":x.id,"incident_id":x.incident_id,"action":x.action,"requested_by":x.requested_by,"approved_by":x.approved_by,"status":x.status,"created_at":x.created_at.isoformat()+"Z"} for x in db.query(ActionApproval).filter_by(tenant_id=p.tenant_id).order_by(ActionApproval.created_at.desc()).limit(100)]

@router.post("/telemetry/ingest",status_code=202)
def ingest(body:IngestRequest,idempotency_key:str=Header(min_length=8,max_length=100),p:Principal=Depends(require_role("ANALYST")),db:Session=Depends(get_db)):
...
def job(job_id:str,p:Principal=Depends(require_role("VIEWER")),db:Session=Depends(get_db)):
...
def bootstrap_rules(db):
    ...
    db.commit()
@router.get("/rules")
def rules(p:Principal=Depends(require_role("VIEWER")),db:Session=Depends(get_db)):bootstrap_rules(db);return [{"id":x.id,"rule_id":x.rule_id,"version":x.version,"name":x.name,"description":x.description,"owner":x.owner,"severity":x.severity,"status":x.status,"enabled":x.enabled,"parameters":json.loads(x.parameters_json)} for x in db.query(DetectionRule).order_by(DetectionRule.rule_id,DetectionRule.version)]
@router.post("/rules",status_code=201)
def create_rule(body:RuleCreate,p:Principal=Depends(require_role("ADMINISTRATOR")),db:Session=Depends(get_db)):
...
def promote_rule(rule_id:str,version:str,body:RuleStatus,p:Principal=Depends(require_role("ADMINISTRATOR")),db:Session=Depends(get_db)):
...
def replay(event_ids:list[str],p:Principal=Depends(require_role("ANALYST")),db:Session=Depends(get_db)):
...
def cursor_events(cursor:str|None=None,limit:int=100,p:Principal=Depends(require_role("VIEWER")),db:Session=Depends(get_db)):
