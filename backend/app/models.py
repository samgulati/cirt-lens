...
from sqlalchemy import String, Integer, DateTime, Text, UniqueConstraint, Index, Boolean
...
class Incident(Base):
    ...
    incident_fingerprint: Mapped[str | None] = mapped_column(String, nullable=True, unique=True, index=True)
...
class DetectionFindingRecord(Base):
    ...
    __table_args__ = (UniqueConstraint("event_id","rule_id","rule_version","flag",name="uq_finding_event_rule_version_flag"),Index("ix_findings_rule_flag","rule_id","flag"))
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
...
class EvidenceBookmark(Base):
    ...
    __table_args__ = (UniqueConstraint("incident_id","event_id",name="uq_bookmark_incident_event"),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    incident_id: Mapped[str] = mapped_column(String, index=True)
    event_id: Mapped[str] = mapped_column(String, index=True)
    analyst: Mapped[str] = mapped_column(String)
...
class ActionApproval(Base):
    ...
    __table_args__=(UniqueConstraint("tenant_id","incident_id","action","status",name="uq_open_action_approval"),Index("ix_approval_tenant_status","tenant_id","status"))
    id:Mapped[str]=mapped_column(String,primary_key=True)
    tenant_id:Mapped[str]=mapped_column(String,index=True)
    incident_id:Mapped[str]=mapped_column(String,index=True)
    action:Mapped[str]=mapped_column(String)
...
class IngestionJob(Base):
    ...
    __table_args__=(UniqueConstraint("tenant_id","idempotency_key",name="uq_job_tenant_idempotency"),Index("ix_job_tenant_status","tenant_id","status"))
    id:Mapped[str]=mapped_column(String,primary_key=True)
    tenant_id:Mapped[str]=mapped_column(String,index=True)
...
class DetectionRule(Base):
    ...
    __table_args__=(UniqueConstraint("rule_id","version",name="uq_rule_version"),Index("ix_rule_status","status"),)
    id:Mapped[int]=mapped_column(Integer,primary_key=True,autoincrement=True)
    rule_id:Mapped[str]=mapped_column(String,index=True)
