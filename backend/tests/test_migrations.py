...
def test_fresh_database_is_created_by_explicit_migrations(tmp_path):
    ...
    unique_sql = connection.execute(
        "SELECT sql FROM sqlite_master WHERE type='table' AND name='detection_findings'"
    ).fetchone()[0]
...
def test_legacy_database_upgrades_without_losing_rows(tmp_path):
    ...
    assert connection.execute("SELECT title FROM incidents WHERE id='INC-LEGACY'").fetchone() == (
        "Legacy incident",
    )
    assert connection.execute("SELECT activity FROM events WHERE id='LEGACY-1'").fetchone() == (
        "Legacy event",
    )
    connection.close()
