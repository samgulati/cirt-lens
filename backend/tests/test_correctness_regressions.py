...
import pytest
from app.models import DetectionFindingRecord, Event, IncidentRiskHistory
...
def test_cohesion_includes_zero_pairs(now):
    a = event("A", now, "alice")
    b = event("B", now, "alice")
    c = event("C", now + timedelta(hours=2), "bob")
    assert group_cohesion([a, b, c]) == pytest.approx(7 / 3)


def test_failed_new_device_cannot_cause_privilege_escalation(now):
    raw = generate_raw_scenario("credential", 9900, now)
    failed = next(e for e in raw if getattr(e, "result", None) == "FAILURE")
    cloud = CloudEventInput(
        id="CLOUD-FAIL",
        timestamp=failed.timestamp + timedelta(minutes=2),
        user=failed.user,
...
@pytest.mark.parametrize(
    "flags,expected",
    [
        ({"IMPOSSIBLE_TRAVEL", "NEW_DEVICE"}, "credential"),
        ({"SUSPICIOUS_POWERSHELL", "CREDENTIAL_ACCESS"}, "endpoint"),
        ({"UNUSUAL_EGRESS", "MASS_DOWNLOAD"}, "exfiltration"),
    ],
)
def test_playbook_selection_matches_classification(flags, expected):
    incident_type, _, _ = infer_incident_type(flags)
...
def test_cross_batch_full_chain_one_enriched_incident(db, now):
    raw = generate_raw_scenario("credential", 9910, now)
    first = raw[:2]
    rest = raw[2:]
    process_raw_telemetry(db, first, BASELINE_DEVICES)
    incidents, _, _ = process_raw_telemetry(db, rest, BASELINE_DEVICES)
...
def test_contained_case_is_not_reopened_by_repeated_scenario(db, now):
    raw = generate_raw_scenario("endpoint", 9915, now)
    first = process_raw_telemetry(db, raw, BASELINE_DEVICES)[0][0]
    first.status = "CONTAINED"
    db.commit()
    repeated = generate_raw_scenario("endpoint", 9916, now)
    second = process_raw_telemetry(db, repeated, BASELINE_DEVICES)[0][0]
    assert (
        second.id != first.id
        and first.status == "CONTAINED"
        and second.status == "NEW"
        and second.incident_fingerprint != first.incident_fingerprint
    )


def test_pipeline_rolls_back_partial_failure(db, now, monkeypatch):
    monkeypatch.setattr(
        "app.services.pipeline.create_or_update_incidents",
        lambda *args: (_ for _ in ()).throw(RuntimeError("forced")),
    )
    with pytest.raises(RuntimeError):
        process_raw_telemetry(db, generate_raw_scenario("endpoint", 9920, now), BASELINE_DEVICES)
