...
export interface Incident {
  ...
  notes: AnalystNote[];
  bookmarks: { id: number; event_id: string; analyst: string; note: string; timestamp: string }[];
  risk_history: RiskHistory[];
  detection_findings: DetectionFinding[];
}
...
export interface AIResponse {
  answer: string;
  mode: 'local_deterministic' | 'openai' | 'local_fallback';
  validated: boolean;
...
export interface DashboardResponse {
  kpis: {
    ...
    detection_findings: number;
    mean_triage_time: string;
  };
  severity: Record<string, number>;
  trend: { hour: string; incidents: number }[];
