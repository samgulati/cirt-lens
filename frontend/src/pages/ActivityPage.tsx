...
export default function ActivityPage() {
  ...
  const load = useCallback(() => {
    void api<ActivityRecord[]>('/activity')
      .then(setItems)
  ...
  return (
    <>
      <PageTitle
        ...
        title="Activity Log"
        sub="Immutable-style trail of every simulated analyst action"
      />
      <div className="panel overflow-x-auto scroll">
        <div className="min-w-[980px]">
          ...
          {items.map((item) => (
            <div key={item.id} className={`grid-table ${columns} gap-4 px-4 text-xs`}>
              <span className="whitespace-nowrap">{fmt(item.timestamp)}</span>
              <span className="min-w-0 truncate" title={item.analyst}>
                {item.analyst}
              </span>
              <span className="min-w-0 truncate font-mono" title={item.action}>
                {item.action}
              </span>
          ...
          {!items.length && (
            <div className="p-12 text-center">
              <Command className="mx-auto muted" />
              <div className="mt-3">No analyst actions yet</div>
              <div className="text-xs muted mt-1">
                Execute a response action to create an audit entry.
              </div>
            </div>
          )}
        </div>
